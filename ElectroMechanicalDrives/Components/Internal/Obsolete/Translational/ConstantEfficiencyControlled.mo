within ElectroMechanicalDrives.Components.Internal.Obsolete.Translational;
model ConstantEfficiencyControlled
  "Efficiency model considering constant efficiency"
  extends Modelica.Icons.ObsoleteModel;
  extends Modelica.Mechanics.Translational.Interfaces.PartialTwoFlangesAndSupport;
  parameter Real efficiency = 0.9 "Efficiency coefficient";
  parameter Modelica.SIunits.Time T = 1E-6
    "Internal controller time constant";
  parameter Modelica.SIunits.Velocity vRef = 1
    "Rereference velocity of controller";
  extends Modelica.Thermal.HeatTransfer.Interfaces.PartialElementaryConditionalHeatPortWithoutT;
Internal.VariableDamper variableDamper(useHeatPort=useHeatPort)
  annotation (Placement(transformation(
      extent={{10,10},{-10,-10}},
      rotation=90,
      origin={0,30})));
  Modelica.Mechanics.Translational.Sensors.PowerSensor powerSensor_a annotation(Placement(transformation(extent = {{-80, 70}, {-60, 90}})));
  Modelica.Mechanics.Translational.Sensors.PowerSensor powerSensor_b annotation(Placement(transformation(extent = {{80, 70}, {60, 90}})));
  ...
  Modelica.Mechanics.Translational.Sensors.PowerSensor powerSensor_loss annotation(Placement(transformation(extent = {{-10, -10}, {10, 10}}, rotation = 270, origin = {0, -38})));
  ...
  Modelica.Blocks.Continuous.Integrator integrator(final initType = Modelica.Blocks.Types.Init.InitialState, final y_start = 0, final k = vRef ^ 2 / T) annotation(Placement(transformation(extent = {{-10, -10}, {10, 10}}, rotation = 90, origin = {-30, 0})));
equation
  lossPower = 0;
