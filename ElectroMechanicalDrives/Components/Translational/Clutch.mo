...
model Clutch "Clutch based on Coulomb friction"
  ...
  parameter Real mu_pos[:, 2] = [0, 0.5]
    "[v,mu] positive sliding friction coefficient (v_rel>=0)";
  parameter Real peak(final min = 1) = 1
    "peak*mu_pos[1,2] = maximum value of mu for v_rel==0";
  parameter Real cgeo(final min = 0) = 1
    "Geometry constant containing friction distribution assumption";
  parameter Modelica.Units.SI.Force fn_max(final min=0, start=1)
  ...
  Modelica.Units.SI.Acceleration a_rel(start=0)
...
equation
  ...
  f0_max = peak * f0;
  // Friction torque
  f = if locked then sa*unitForce else if free then 0 else cgeo*fn*(
    if startForward then
      Modelica.Math.Vectors.interpolate(mu_pos[:,1], mu_pos[:,2], v_rel, 1)
